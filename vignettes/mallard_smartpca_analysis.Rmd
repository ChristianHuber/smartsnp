---
title: "Example PCA and permutation analysis"
output: rmarkdown::html_vignette
description: >
  This Vignette provides an example analysis of genetic data using 
  the smartsnp package.
vignette: >
  %\VignetteIndexEntry{Example PCA and PERMANOVA analysis}
...
legend("topleft", legend = levels(as.factor(my_groups)), cex = 1, pch = 21, 
       pt.cex = 1.5, col = "black", pt.bg = cols, text.col = cols)
```
...
## 6) Run smart_permanova (missing values with means, control for genetic drift, global and pairwise tests applied)
...
Show PERMANOVA table (global and pairwise):
...
Low p value implies weak support of the null hypothesis.

```{r}
head(permanovaR$permanova.pairwise_test)
```

The lowest p values result from pairwise comparisons consistently occur between the Greenland and the remaining flyways, supporting a unique SNP composition in Greenland mallards

## 7) Run smart_permdisp (missing values with means, control for genetic drift, global and pairwise tests applied, dispersion adjusted to control for sample size per group)

Run PERMDISP test (group dispersion in PCA1 x PCA2 space) and assign results to object permdispR heteroscededasticity tests in combination with ANOVA tests tells whether ANoVA tests are driven by mean and/or varinance differences among groups in a multivariate context, location and dispersion are analogous with mean and variance in a univariate context as number per individuals per flyway differ a lot among flyways, PERMDISP is run to control for sample-size bias (samplesize_bias = TRUE).
...
Pairwise tests indicate that PERMANOVA tests mainly captured differences in location the lowest p values occur for comparisons of the Europe North Western (ENW) flyway with the other flyways, supporting that ENW individuals show the greatest dispersion in the PCA1 x PCA2 ordination.


## 8) Run smart_mva (missing values with means, control for genetic drift, dispersion adjusted to control for sample size per group)

Run PCA, and PERMANOVA and PERMDISP tests (group location and dispersion in PCA1 x PCA2 space), and assign results to object mvaR. No pairwise comparisons are applied (default: pairwise = "FALSE"), so computation will be relatively fast
...
mvaR has three elements (data, pca, test):

```{r}
names(mvaR)
```

Get scaled dataset (none, covariance, correlation, drift):
...
Get PERMANOVA and PERMDISP results:
